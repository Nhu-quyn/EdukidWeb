...
const ImageChooseWord = ({ questionContent, image, options, answer }) => {
      ...
      <Content>
        ...
        <OptionsWrapper>
          {options.map((option, index) => {
            // Determine style based on selection and check status
            let bgColor = "#fff";
            let borderColor = "#ccc";
            if (hasChecked) {
              if (option === answer) {
                bgColor = "#d4edda"; // correct: light green
                borderColor = "#28a745";
              } else if (selected === option && option !== answer) {
                bgColor = "#f8d7da"; // incorrect: light red
                borderColor = "#dc3545";
              }
            } else if (selected === option) {
              bgColor = "#e2e6ea"; // selected but not checked
              borderColor = "#007bff";
            }
